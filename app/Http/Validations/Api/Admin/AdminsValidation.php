...
class AdminsValidation
{
    public function store()
    {
        ...
        return [
            'rules' => [
                'name'            => 'required|between:3,25|regex:/^[A-Za-z0-9\-\_]+$/|unique:admins,name,' . request()->route('admin.id'),
    ...
    public function update()
    {
        return [
            'rules' => [
                'name'            => 'nullable|between:2,25|unique:admins,name,' . request()->route('admin.id'),
            ...
            'messages' => [
                'name.between'       => '名称必须介于 2 - 25 个字符之间。',
                'name.unique'        => '名称已被占用，请重新填写',
                'password.min'       => '密码长度不能小于6位',
                'email.email'        => '邮箱格式不正确',
                'email.unique'       => '邮箱已经被使用',
